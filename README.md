...
- **AI 站长** — 配置 OpenAI 兼容 AI（DeepSeek/GLM/OpenAI/Moonshot/千问，或自定义），启用后可实现评论自动回复（Agent 工具循环）、AI 评论审核（批量）、文章 AI 润色，并提供操作记录、多模态模型与视觉辅助、Tavily 联网搜索、每日配额/风控
...
- **评论自动回复（Agent 工具循环）** — 用户发表评论后，AI 后台自动回复；若用户回复 AI，AI 会结合文章内容与对话历史持续交流（用户之间的回复不会触发 AI）。回复经由一个可规划多步的 Agent 循环驱动（默认最多 10 轮、整体超时 5~10 分钟）：AI 可随时直接输出，也可**静默调用工具**获取信息后再作答，且不会向用户提及工具存在。
- **AI 评论审核（批量）** — 自动检测广告、辱骂、违法等违规内容，命中后自动禁用该评论（非删除，可恢复）。未审核评论按「批量间隔」合并为一次 AI 调用统一审核，显著降低 token 消耗。
...
在「功能分配」中将「视觉辅助」指定为某个开启多模态的供应商后，`get_article_image` 会用该模型真正"看图"并返回图片内容描述。未配置时该工具会返回提示，不影响其它功能。图片地址兼容本地相对路径（自动拼接站点域名）、七牛云 CDN 与外部 https。

### MCP（Tavily 联网搜索）

在 `/admin/ai → MCP 配置` 填入 Tavily API Key 保存，系统会先测试连通性、通过后才启用。启用后 AI 在需要准确/实时信息时自动联网搜索并（可选）提取网页内容。
