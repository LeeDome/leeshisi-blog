...
一个轻量化的个人技术博客系统，支持文章发布（Markdown）、图册管理、访客评论、文章评分/点赞、评论投票、图片上传（本地/七牛云可选）、账号设置、搜索、归档等功能。基于 Node.js + Express + SQLite 构建。
...
- **文章点赞** — 按 IP 去重，防止刷赞
- **搜索** — 文章全文搜索
- **侧边栏** — 作者信息、最近评论、热门文章、归档、标签云、运行时间
- **管理后台** — 仪表盘、文章/分类/标签/图册/评论/页面管理、站点设置（含七牛云配置）、账号设置（昵称/邮箱/密码修改）
